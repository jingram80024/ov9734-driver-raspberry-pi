...
/* Modified original driver by Tianshu Qiu and Bingbu Cao to remove ACPI driver loading and use of_table instead
/  
*/
...
static int ov9734_link_freq_check(struct device *dev)
{
...
static int ov9734_parse_dt(struct ov9734 *ov9734, struct device_node *np)
{
	struct v4l2_fwnode_endpoint bus_cfg = {
		.bus_type = V4L2_MBUS_CSI2_DPHY,
	};
	...
	/*ov9734->clock_ncont = bus_cfg.bus.mipi_csi2.flags &
					V4L2_MBUS_CSI2_NONCONTINUOUS_CLOCK;*///in ov5647 this was used to control mipi formatting see ov5647.c
...
static int ov9734_probe(struct i2c_client *client)
{
	...
	// pass device tree nodes to v4l2 for parsing
	if (IS_ENABLED(CONFIG_OF) && client->dev.of_node) {
		ret = ov9734_parse_dt(ov9734, client->dev.of_node);
		if (ret) {
	...
	// get clock frequency from of_node
	ov9734->xclk = devm_clk_get(&client->dev, NULL);
	if (IS_ERR(ov9734->xclk)) {
		dev_err(&client->dev, "could not get xclk");
		return PTR_ERR(ov9734->xclk);
	}

	// check that clk rate for this driver matches whats in the .dts
	xclk_freq = clk_get_rate(ov9734->xclk);
	if (xclk_freq != 25000000) {
		dev_err(&client->dev, "xclk frequency %u not supported, must be 250000", xclk_freq);
		return -EINVAL;
	}

	ret = ov9734_link_freq_check(&client->dev);
	if (ret) {
		dev_err(&client->dev, "link frequency not compatible: %d",
			ret);
		return ret;
	}
	// get gpio
	// conigure regulators
	dev_err(&client->dev, "HARDWARE CHECKS PASSED, v1.2 ready");
	...
	//need to turn on and check that it is responding
...
MODULE_AUTHOR("Jonathan, Ingram <jonathan.ingram225@gmail.com>");
